...
class GameManager{
    ...
    public function joinWithBot(Player $player){
        ...
        if(count($this->teams) < 2){
            $player->sendMessage('§cNeed at least 2 teams in config for bot mode.');
            return;
        }
        ...
        $this->broadcast('§eBot mode enabled. Opponent: §c' . $this->botName);
    }
    ...
    private function assignTeams(){
        ...
        if($this->botMode && count($this->players) === 1 && count($teamIds) >= 2){
            $player = array_values($this->players)[0];
            $pn = strtolower($player->getName());
            $this->teamOf[$pn] = $teamIds[0];
            $this->botTeamId = $teamIds[1];

            $player->sendMessage('§7You are in team ' . $this->teams[$teamIds[0]]->getColoredName());
            $this->broadcast('§c' . $this->botName . ' §7joined team ' . $this->teams[$this->botTeamId]->getColoredName());
            return;
        }
    ...
    private function tickBotMode(){
        ...
        $this->broadcast('§c' . $this->botName . ' §escored against ' . $this->teams[$target]->getColoredName() . '§e! §7(' . $this->scores[$this->botTeamId] . '/' . $this->plugin->getMaxScore() . ')');

        if($this->plugin->isParticlesEnabled()){
            $pos = $this->teams[$this->botTeamId]->getSpawn();
            $pos->getLevel()->addParticle(new HappyVillagerParticle($pos));
        }
